...
func (w *Params) ReconcileResources(clientset kubernetes.Interface) error {
	...
	if w.shouldRenewCertificates(webhookServerSecret, webhookConfiguration) {
		...
		if _, err := clientset.AdmissionregistrationV1beta1().ValidatingWebhookConfigurations().Update(context.TODO(), webhookConfiguration, metav1.UpdateOptions{}); err != nil {
			return err
		}
		...
		if _, err := clientset.CoreV1().Secrets(w.Namespace).Update(context.TODO(), webhookServerSecret, metav1.UpdateOptions{}); err != nil {
			return err
		}
	}
